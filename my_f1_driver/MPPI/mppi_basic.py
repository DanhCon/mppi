...
class MPPIController(Node):
    def __init__(self):
        ...
        self.sub_clear_virtual = self.create_subscription(Empty, "/clear_virtual_obstacles", self.clear_virtual_obstacles_callback, 10)
    ...
    def lidar_callback(self, msg: LaserScan) -> None:
        ...
        try:
            ...
            self.obstacle_stamp = rclpy.time.Time.from_msg(msg.header.stamp)
        except Exception:
    ...
    def _compute_cost(
        ...
        dist_wp   = np.linalg.norm(delta_wp, axis=-1)                  # (N,T,W)
        min_wi    = np.argmin(dist_wp, axis=2)                         # (N,T)  int
        min_dists = dist_wp[np.arange(N)[:, None],
                             np.arange(T)[None, :],
                             min_wi]                                    # (N,T)
        ...
        # progress: hiện max thay vì mean — mean = 0 do nhiễu dương/âm triệt tiêu
        # max cho thấy rollout tốt nhất tiến được bao xa
        ...
        # Lưu lại để odom_callback trích xuất của best rollout
    ...
    def odom_callback(self, msg: Odometry) -> None:
        ...
        # Bơm target speed vào bước cuối cùng của horizon để dẫn dắt tốc độ phục hồi
        ...
        # Tính toán n_eff, cost_std và khoảng cách chướng ngại vật gần nhất
        ...
        # Tìm index của rollout tốt nhất và trích xuất thành phần cost
        ...
        # Cảnh báo không ổn định (n_eff cực thấp, hoặc có va chạm ở rollout tốt nhất, hoặc ở quá gần tường)
        is_unstable = (n_eff < 5) or (beta > 5000.0) or (min_obs_dist < self.robot_radius + 0.05)
        
        # In log định kỳ (mỗi 10 chu kỳ) hoặc cảnh báo khẩn cấp (giới hạn tần suất tối đa 1 lần mỗi 4 chu kỳ ~ 0.2s)
        ...
        # 11. Receding horizon đã được thực hiện trong odom_callback (sau bước 7)
